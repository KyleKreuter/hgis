...
describe('AttributeTable', () => {
  ...
  /*
   * The two below are `test.fails`: the rule they state is the right one, and the
   * component does not yet keep it -- a sort or search set on one layer is still sent
   * for the next one. `test.fails` records that as the known state without leaving the
   * suite red. Each turns red again the moment the component is fixed, which is the
   * signal to drop the `.fails` and keep the plain assertion.
   *
  ...
  test.fails('drops the previous layer"s sort on a layer switch', async () => {
  ...
  test.fails('drops the previous layer"s search term on a layer switch', async () => {
    const { calls, switchToLayerB, user } = setup()
