...
describe('previewTilesFor', () => {
  it('liefert ein leeres Feld ohne center und ohne extent', () => {
    expect(
      previewTilesFor({ center: null, zoom: null, extent: null, basemap: 'osm' }, TEST_BASEMAP_CATALOG),
    ).toEqual([])
  })

  it('liefert ein leeres Feld ohne Ortsangabe, auch wenn zoom gesetzt ist', () => {
    expect(
      previewTilesFor({ center: null, zoom: 8, extent: null, basemap: 'osm' }, TEST_BASEMAP_CATALOG),
    ).toEqual([])
  })
  ...
  it('liefert ein leeres Feld bei basemap: none, obwohl ein center vorliegt', () => {
    expect(
      previewTilesFor(
        { center: [10, 50], zoom: 10, extent: null, basemap: 'none' },
        TEST_BASEMAP_CATALOG,
      ),
    ).toEqual([])
  })
  ...
  it('begrenzt den Zoom auf das, was OSM hergibt', () => {
    const tiles = previewTilesFor(
      { center: [0, 0], zoom: 25, extent: null, basemap: 'osm' },
      TEST_BASEMAP_CATALOG,
    )
  ...
  it('errechnet aus einer bekannten Koordinate die erwartete Kachel', () => {
    ...
    const tiles = previewTilesFor(
      { center: [0, 0], zoom: 4, extent: null, basemap: 'osm' },
      TEST_BASEMAP_CATALOG,
    )
