...
/**
 * Lets an existing layer's attribute fields be renamed and new ones added -- the second
 * way fields ever change after a layer exists, next to `CreateLayerDialog`'s one-time
 * set at creation (CONTRACT.md "Attributfelder hinzufügen und umbenennen").
 *
 * Deliberately does not offer deleting a field, changing its type or reordering it --
 * out of scope by the contract, and each would touch the physical column this dialog
      ...
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {layer && (
              <>
                Attributfelder von „{layer.name}" umbenennen oder neue ergänzen. Typ und
                Reihenfolge stehen fest, sobald ein Feld angelegt ist.
              </>
            )}
          </DialogDescription>
        </DialogHeader>
              ...
              <Table>
                <TableHeader className="sticky top-0 z-10 bg-popover">
                  <TableRow>
                    ...
                    <TableHead className="h-7 w-9 text-xs" />
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {fields.map((field) => (
